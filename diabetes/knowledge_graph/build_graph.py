# ==============================================================================
# SCRIPT PYTHON NẠP KNOWLEDGE GRAPH TIỂU ĐƯỜNG VÀO NEO4J DATABASE
# Nguồn dữ liệu: Tri thức y khoa Nhà Thuốc Long Châu (nhathuoclongchau.com.vn)
# ==============================================================================
...
def build_graph(uri, user, password, cypher_file):
    try:
        ...
        print("ℹ️ Bạn vẫn có thể copy trực tiếp nội dung trong file 'diabetes_ontology.cypher' dán vào Neo4j Browser để chạy.")
        return False
    ...
    with driver.session() as session:
        for idx, query in enumerate(statements, 1):
            try:
                session.run(query)
                print(f"  [{idx}/{len(statements)}] Đã thực thi lệnh tạo thực thể/quan hệ...")
        ...
        print(f"\n🎉 HOÀN TẤT NẠP KNOWLEDGE GRAPH! Tổng số Nodes: {node_count}, Relationships: {rel_count}")

    driver.close()
    return True

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build Diabetes Knowledge Graph in Neo4j (Long Chau Source)")
    ...
    cypher_path = os.path.join(script_dir, "diabetes_ontology.cypher")
